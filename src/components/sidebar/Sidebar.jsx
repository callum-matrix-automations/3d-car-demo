    ...
    <div className="hidden md:flex absolute left-6 top-[calc(50%+100px)] z-10 flex-col gap-3">
      {icons.map(({ label, path }) => (
        <button
          key={label}
          title={label}
          className="w-10 h-10 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all duration-200 cursor-pointer"
        >
